...
<body class="pl-64 bg-gradient-to-br from-blue-50 to-gray-100 font-sans leading-normal tracking-normal">
    <div class="container mx-auto px-4 py-6">
        ...
        <!-- Form Filter Data Kendaraan -->
        <div class="flex justify-center mb-6">
            <form action="{{ route('kendaraan.index') }}" method="GET" class="flex space-x-2">
                <input type="text" name="search" placeholder="Cari berdasarkan no plat, merk atau tipe" 
                       value="{{ $search }}" 
        ...
        <!-- Tampilkan data kendaraan hanya jika sudah ada filter -->
        @if($search)
            ...
            @else
                <p class="text-center text-gray-600">Data kendaraan tidak ditemukan untuk filter: <strong>{{ $search }}</strong></p>
            @endif
        @else
            <p class="text-center text-gray-600">Silahkan masukkan filter pencarian untuk menampilkan data kendaraan.</p>
        @endif
    </div>
</body>
<script>
