...
<body class="pl-64 bg-gradient-to-br from-blue-50 to-gray-100 font-sans leading-normal tracking-normal">
    <div class="container mx-auto px-4 py-6">
        <!-- Judul Halaman -->
        <h1 class="text-2xl font-bold mb-4 text-center text-blue-800">Daftar Sparepart</h1>

        <!-- Container Modal untuk Tambah Sparepart -->
        <div x-data="{ showCreate: false }">
            ...
            <!-- Modal Pop-Up untuk Tambah Sparepart -->
            <div x-show="showCreate" class="fixed inset-0 flex items-center justify-center z-50">
                ...
                <div class="bg-white w-full max-w-md p-6 rounded-lg shadow-lg z-10">
                    <!-- Sertakan Form Create Sparepart -->
                    @include('backend.sparepart.create')
        ...
        <div class="bg-white shadow-sm overflow-hidden border border-gray-300 mt-4">
            <table id="tableSparepart" class="table-auto w-full border-collapse text-sm">
                <thead>
                    <tr class="bg-gradient-to-r from-blue-200 to-teal-200 text-gray-800">
                        ...
                        <th class="border px-3 py-2 text-left font-medium">Stock</th>
                        <th class="border px-3 py-2 text-left font-medium">Harga</th>
                ...
                <tbody>
                    @foreach ($spareparts as $sparepart)
                        <tr class="hover:bg-blue-50 transition duration-200">
                            ...
                            <td class="border px-3 py-2 text-gray-600">{{ $sparepart->nama }}</td>
                            <td class="border px-3 py-2 text-gray-600">{{ $sparepart->stock }}</td>
                            <td class="border px-3 py-2 text-gray-600">Rp {{ number_format($sparepart->harga, 2, ',', '.') }}</td>
                            <td class="border px-3 py-2 text-gray-600">{{ $sparepart->deskripsi }}</td>
                            <td class="border px-3 py-2 text-gray-600">
                                @if($sparepart->image)
                                    <img src="{{ asset('storage/' . $sparepart->image) }}" alt="{{ $sparepart->nama }}" width="50">
                                @else
                                    -
                                @endif
                            </td>
                            <td class="border px-3 py-2 text-center">
                                <div class="flex justify-center space-x-4">
                                    ...
                                    <div x-data="{ showEdit: false }">
                                        <!-- Tombol Trigger Modal untuk Edit -->
                                        <button @click="showEdit = true"
                                                class="bg-gradient-to-r from-teal-400 to-blue-400 text-white font-semibold px-6 py-3 rounded-lg shadow-md hover:from-teal-500 hover:to-blue-500 focus:ring focus:ring-blue-300 transition-all duration-300">
                                            Edit Sparepart
                                        </button>

                                        <!-- Modal Pop-Up untuk Edit Sparepart -->
                                        <div x-show="showEdit" class="fixed inset-0 flex items-center justify-center z-50">
                                            ...
                                            <!-- Konten Modal -->
                                            <div class="bg-white w-full max-w-md p-6 rounded-lg shadow-lg z-10">
                                                <!-- Sertakan Form Edit Sparepart dan kirim data sparepart -->
                                                @include('backend.sparepart.edit', ['sparepart' => $sparepart])
                                    ...
                                    <!-- Tombol Hapus -->
                                    <form action="{{ route('sparepart.destroy', $sparepart->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
